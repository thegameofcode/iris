 ...
 * @Example iris.ApplyBE("ffa/ui/be_validator.js", [_UI1, _UI2, _$DOM1]);
 ...
 * 
 * If you want to add a UI to a validator BE, you have to consider:
 * 	- The UI must have a public Validate method. If this method doesn't return TRUE or dosen't exists, the BE considers that the UI doesn't validate.
 ...
 *  - The DOM element must have a data-validate attribute with any validation type supported by this BE. If not, the element always pass the validation
...
iris.BE(
	...
	function( self ) {
		...
		// LyfeCycle
		self.Apply = function() {
			iris.D("Behaviour Validator Applied");
		};
			...
			for ( var f=0, F=self.Size(); f < F; f++ ) {
				...
				if( _IsUiComponent( el ) ){
					...
					if( el.data("validate") ){
						...
						var f,F = validations.length;
						for( f=0; f<F; f++){
							var validation = validations[f];
						...
						iris.W("[BE validator] DOM element without a data-validation attribute");
					}
				}
			}
			return allValid;
